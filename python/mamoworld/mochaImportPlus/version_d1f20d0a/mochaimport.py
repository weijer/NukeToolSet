# Python bytecode 2.6 (62161) disassembled from Python 2.7
# Embedded file name: /Users/mmohl/AEscripts/MochaImportNuke/download package/mamoworld/01_mochaImportPlus/version_d1f20d0a/mochaimport.py
# Compiled at: 2015-04-20 21:45:22
__all__ = ['createStabilizedView',
 'createCornerPin',
 'createTracker3Node',
 'createTracker4Node',
 'createRotoPaintNodeMI',
 'createRotoNodeMI',
 'createGridWarpNodeMI',
 'createSplineWarpNodeMI',
 'createTransformNodeMI',
 'createCameraAndPointCloud',
 'createCameraRig',
 'setUseGizmos',
...
class dn(l.QDialog):
    ...
    def initUI(a):
        ...
        do = l.QHBoxLayout()
        am = l.QLabel("If you agree to the above terms press 'I Accept'. Otherwise press 'Cancel' to cancel installation of the license.")
        do.addWidget(am)
        ci = l.QHBoxLayout()
...
class dr(l.QDialog):
    ...
    def __init__(a, wwqe, toolVersion, fi):
        ...
        a.toolVersion = toolVersion
        a.wehsrhv = fi
        a.copyRightInfo = '(c) 2014 by CGHotman.com'
        a.initUI()
    ...
    def ghjkh(a, h):
        ...
        if h['status'] == 'valid':
            ...
            am += '\xe5\xb7\xb2\xe5\xae\x89\xe8\xa3\x85\xe6\x97\xa0\xe9\x99\x90\xe5\x88\xb6\xe7\x9a\x84\xe8\xae\xb8\xe5\x8f\xaf\xe6\x8e\x88\xe6\x9d\x83\xef\xbc\x81\n'
            am += '\xe5\x9c\xa8\xe5\xae\x8c\xe6\x95\xb4\xe6\xa8\xa1\xe5\xbc\x8f\xe4\xb8\xad\xe8\xbf\x90\xe8\xa1\x8c\xef\xbc\x8c\xe6\x9c\x89\xe9\x97\xae\xe9\xa2\x98\xe8\xaf\xb7\xe8\x81\x94\xe7\xb3\xbb\xe8\x99\x9a\xe6\x8b\x9f\xe7\x8e\xb0\xe5\xae\x9e\xef\xbc\x81CGHotman@163.com'
        return am

    def rjrkkt(a):
        aa = b.getInput('No license installed for ' + a.wwqe + "\nEnter a valid serial number or click 'cancel' run in trial mode")
        if not aa:
            return None
            a.wehsrhv.zzdfger(aa) and dq(aa, a.wwqe)
        else:
            a.wehsrhv.ddsjz(aa)
            h = a.wehsrhv.ertzz()
            if h['status'] == 'valid':
    ...
    def openTutorial(a):
        fm.open('http://CGHotman.com:808/mocha/tutorial-mochaimport.mp4')
...
def fl(by):
    ...
    if by == 'BTA':
        ...
        return by == 'REN' and 'render only license'
    return 'unknown license'


def dq(fn, wwqe):
    dw = 'https://license.CGHotman.com/exchange/?serial=' + fn
...
class bz(fq):
    ...
    def __parseMochaNukeData(a, j):
        ...
        def bC(fw, j):
            ...
            cp = fx.search(j)
            if cp == None:
                raise cornerPinData___FileFormatError("invalid tracking data - please use corner pin data exported from mocha Pro with the format set to 'Nuke Corner Pin (*.nk)'")
            fy = cp.group(1)
            fz = cp.group(2)
            fA = {'x': dA(fy),
             'y': dA(fz)}
    ...
    def __checkDemoLimitation(a):
        cq = dz()
        if cq.asgg(quiet=False, wegtzeke=a.wegtzeke):
...
def cornerPinNode___loadTrackingDataFromFile():
    ...
    except cornerPinData___FileFormatError as :
...
def fF(e, A):
    s = [e[0] * A[0] + e[1] * A[1] + e[2] * A[2], e[3] * A[0] + e[4] * A[1] + e[5] * A[2], e[6] * A[0] + e[7] * A[1] + e[8] * A[2]]
    return s
...
def cu(d, referenceFrame):
    ...
    for c in range(0, len(t)):
        ...
        m = fC(t[af][1], J[af][1], t[c][1], J[c][1], K[af][1], L[af][1], K[c][1], L[c][1], M[af][1], N[af][1], M[c][1], N[c][1], V[af][1], W[af][1], V[c][1], W[c][1])
        dK.setKey((S, m))
...
def fI(dL, dM):
    ...
    raise Exception("tracking data doesn't contain tracking data for frame " + str(dM) + '.\nPlease choose another reference frame.')
...
def dP(dQ):
    assert '\n' not in dQ
    template = ' \nif not locals().has_key("mochaimport"):\n    nuke.message("Please install MochaImport+ for NUKE (by CGHotman.com) to use this function")\nelse:\n    {myCommand}\n'
    return template.format(myCommand=dQ)


def fQ():
    U = b.createNode('Tracker3')
    be(U, 'mochaimport.trackerNodeMI___loadTracker3TrackingDataFromFilePopup(nuke.thisNode())', 'mochaimport.trackerNodeMI___loadTracker3TrackingDataFromClipboard(nuke.thisNode())')
...
def fR():
    ...
    be(o, 'mochaimport.trackerNodeMI___loadTracker4TrackingDataFromFilePopup(nuke.thisNode())', 'mochaimport.trackerNodeMI___loadTracker4TrackingDataFromClipboard(nuke.thisNode())')
    for _a in range(0, 4):
        o['add_track'].execute()
...
def gj():
    ...
    be(g, 'mochaimport.transformNodeMI___loadTransformTrackingDataFromFilePopup(nuke.thisNode())', 'mochaimport.transformNodeMI___loadTransformTrackingDataFromClipboard(nuke.thisNode())')
    for cG in ['translate',
     'rotate',
     'scale',
     'skew']:
...
def cH(g, u):
    if u.hasPositionData():
        if g['importtranslatedata'].getValue():
        ...
        if u.hasScaleData():
            ...
            aB = u.hasRotationData() and g['importrotatedata'].getValue() and Q(u.getRotationValues())
            g['rotate'].clearAnimated()
            g['rotate'].setAnimated()
            g['rotate'].animation(0).addKey(aB)
        gn = u.hasShearData() and g['importskewdata'].getValue() and Q(u.getShearValues('x'))
...
def cJ(gu):
    E = b.createNode(gu)
    be(E, 'mochaimport.RotoPaintNodeMI___loadTransformMatrixFromFilePopup(nuke.thisNode())', 'mochaimport.RotoPaintNodeMI___loadTransformMatrixFromFromClipboard(nuke.thisNode())')
    cK = cI(E['curves'])
    cL = ej(cK)
...
def gy():
    ...
    be(bY, 'mochaimport.RotoPaintNodeMI___loadTransformMatrixFromFilePopupGridWarp(nuke.thisNode())', 'mochaimport.RotoPaintNodeMI___loadTransformMatrixFromFromClipboardGridWarp(nuke.thisNode())')
    el(bY)
...
def eq(q):
    ...
    br['help'].setValue('renders the point cloud imported from the mocha camera track. Each tracked layer contributes 5 points (four corners + center of surface rectangle)')
    P['help'].setValue('camera imported from mocha camera track')
    aq['help'].setValue('use this node to orient the camera track in your scene as desired (e.g. make the ground plane horizontal etc...)')
...
def gJ():
    aH = b.selectedNodes()
    if not aH:
        return b.nodes.BackdropNode(tile_color=int('%02x%02x%02x%02x' % (232.05, 145.095, 0, 255), 16), note_font_color=4294967040L, note_font_size=36, name='MochaImport+')
    ...
    es += gK - eu
    et += gL - ev
    n = b.nodes.BackdropNode(xpos=cV, bdwidth=es, ypos=cW, bdheight=et, tile_color=int('%02x%02x%02x%02x' % (232.05, 145.095, 0, 255), 16), note_font_color=4294967040L, note_font_size=36, name='MochaImport+')
...
def db(r):
    ...
    """
    ey = gQ(r)
    gR = b.tcl('global no_gizmo; set no_gizmo 1; in %s {%s -New} ; return [value [stack 0].name]' % (ey.fullName(), r.Class()))
...
def createStabilizedView():
    ...
    cP(dd)
    de.setSelected(True)
    eB = b.nodes.BackdropNode(xpos=y.xpos() + cd / 2, bdwidth=cd * 1.5, ypos=y.ypos() - ez, bdheight=df + 2 * ez, tile_color=int('%02x%02x%02x%02x' % (232.05, 145.095, 0, 255), 16), note_font_color=4294967040L, note_font_size=36, name='MochaImport+')
...
def applyMochaDataToNode(node, cornerpinData, referenceFrame=1, layerIndex=0):
    ...
    if node.Class() == 'Group':
        if node.knob('pin1') == None or node.knob('pin2') == None or node.knob('pin3') == None or node.knob('pin4') == None:
            raise MiUnsupportedNodeTypeError("Can only apply tracking data to groups, if they have the knobs 'pin1', 'pin2', 'pin3' and 'pin4'")
        d = bz(cornerpinData)
        u = node.Class() == 'Transform' and bV(d)
        cH(node, u)
